...
class CustomAcquisitionFunction(AnalyticAcquisitionFunction):
    ...
    def forward(self,thetas):
        ...
        for theta in thetas:
            ...
            # variance of gradient before adding imaginary data point
            ...
            # mean = KxX_dx @ K_XX_inv @ Y # graident of posterior mean
            ...
            # variance of gradient after adding imaginary data point
            ...
            # acq_val = log_det/float(cost_model(theta)[0][0]) # cost aware entropy
...
def generate_initial_data(seed = None, fun = None, dim=-1):
    ...
    # X_te_normalized = 0.2+0.6*lhs(dim, samples = N_test)
...
if __name__ == '__main__':
    ...
    replicate = 20
    initial_cost = 130
    lb = np.array([-1]*(dim)) # lb for Rosenbrock
    ub = np.array([1]*(dim)) # ub for Rosenbrock
    bounds = torch.tensor([[0.0] * dim, [1.0] * dim], **tkwargs) # bounds for optimizing acquisition    
    bound_cat = torch.tensor([1.0,3.0]) # bound for categorical variable
    ...
    for seed in range(replicate):
        ...
        for i in range(N_ITER):
            ...
            for I in range(N_iner):
                ...
                candidate[:,0:dim][candidate[:,0:dim]>1] = 1
                candidate[:,0:dim][candidate[:,0:dim]<0] = 0
            ...
            theta[:,0:dim][theta[:,0:dim]>1] = 1
            theta[:,0:dim][theta[:,0:dim]<0] = 0
            
            theta_rescale = theta.clone()
            theta_rescale[:,0:dim] = torch.tensor(lb).to(device) + (torch.tensor(ub-lb)).to(device)*theta_rescale[:,0:dim]
            new_obj = fun(theta_rescale.clone()).unsqueeze(1).to(device)
            ...
            cumulative_cost += cost
            cost_list[seed].append(int(cumulative_cost))
            best_Y_list[seed].append(max(best_Y_list[seed][-1],float(new_obj[0])))
            
            print(new_obj)
            if cumulative_cost>300 or float(new_obj[0])>0.999:
                break
    ...
    # Save results
    np.save('Cartpole_cost_CAGES_MFBO.npy',xx)
    np.save('Cartpole_reward_CAGES_MFBO.npy',yy)