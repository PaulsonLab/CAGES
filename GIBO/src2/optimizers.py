...
class VanillaBayesianOptimization(AbstractOptimizer):
    ...
    def __init__(
        self,
        params_init: torch.Tensor, # starting sampling point
        objective: Callable[[torch.Tensor], torch.Tensor], # objective function
        Model,
        model_config: Dict,
        ...
        self.x_min = x_min
        self.x_max = x_max  
        self.train_x = (train_x_init-x_min)/(x_max-x_min) # normalized scale
        
        self.train_y = train_y_init
        self.mean_y = train_y_init.mean()
        self.std_y = train_y_init.std()
        self.train_y_init_scale = (train_y_init-self.mean_y)/self.std_y # standardize scale
        ...
        self.model = Model(self.train_x, self.train_y_init_scale, **model_config)
        ...
        #     print('GP hyperparameters optimized!')
    ...
    def step(self) -> None:
        ...
        #new_x,acq_value = self.optimize_acqf(self.acquisition_function(self.model, **self.acqf_config),**self.optimize_acqf_config)
        
        self.bounds = torch.tensor([[0]*self.D,[1]*self.D], dtype=torch.float64)
        new_x,acq_value = self.optimize_acqf(self.acquisition_function(self.model,best_f=self.model.train_targets.max()), bounds = self.bounds)
        
        new_x_rescale = new_x*(self.x_max-self.x_min)+self.x_min # rescale sampling point
        new_y = self.objective(new_x_rescale)
        self.func_eval_iteration+=1
        self.params = new_x.clone()

        # Update training points.
        #train_x = torch.cat([self.model.train_inputs[0], new_x])
        self.train_x  = torch.cat([self.train_x, new_x]) # normalized scale
        # train_y = torch.cat([self.model.train_targets, new_y])
        self.train_y = torch.cat([self.train_y, new_y]) # original scale
        self.train_y_scale = (self.train_y-self.train_y.mean())/self.train_y.std() # standardize
        
        self.model.set_train_data(inputs=self.train_x, targets=self.train_y_scale, strict=False) # updata the GP with new sampling point
...
class BayesianGradientAscent(AbstractOptimizer):
    ...
    def __init__(
        ...
        self.func_eval_iteration = 0 # number of function evaluation
        ...
        # Add initial training data to the GP model
        self.model.append_train_data(self.train_x, self.train_y_init_scale)
    ...
    def step(self) -> None:
        ...
        self.params_rescale = self.params*(self.x_max-self.x_min)+self.x_min
        
        f_params = self.objective(self.params_rescale)
        self.func_eval_iteration+=1
        ...
        self.train_y = torch.cat([self.train_y, f_params])
        f_params_scaled = (f_params-self.train_y.mean())/self.train_y.std()
        self.model.append_train_data(self.params, f_params_scaled)
        ...
        #if len(self.params_history_list)!=1:
        ...
        # switch the aacquisition function if local get stuck
        ...
        for i in range(inner_iteration):
            ...
            new_x, acq_value = self.optimize_acqf(self.acquisition_fcn, self.bounds) # use gradient information acquisition function
            
            new_x_rescale = new_x*(self.x_max-self.x_min)+self.x_min # rescale sampling point
            new_y = self.objective(new_x_rescale)
            self.func_eval_iteration+=1
            self.params_history_list.append(new_x)
            #print('new_y=', new_y)
            ...
            new_y_scaled = (new_y-self.train_y.mean())/self.train_y.std()
            self.model.append_train_data(new_x, new_y_scaled)
        ...
        self.move(method="step") # update params by gradient ascent
    ...
    def move(self, method):
        if method == "step":
            with torch.no_grad():
                ...
                params_grad = -mean_d.view(1, self.D) # add the minus term because we are doing gradient ascent
                ...
                self.optimizer_torch.step() # update sampling point
                #self.iteration +=1
                # self.params = self.params-params_grad*0.25
...
class BayesianGradientAscent_custom(AbstractOptimizer):
    """Optimizer for Bayesian gradient ascent.
    ...
    def __init__(
        ...
        """Inits optimizer Bayesian gradient ascent."""
        super(BayesianGradientAscent_custom, self).__init__(params_init, objective)
        ...
        self.func_eval_iteration = 0 # number of function evaluation
        # self.train_x_init = train_x_init
        # self.train_y_init = train_y_init
        ...
        self.model = Model(self.D, normalize, unnormalize, **model_config) # Define Jacobian GP model
        self.model_exactGP = ExactGPSEModel(self.train_x, self.train_y_init_scale,ard_num_dims=self.D) # Define Exact GP model
        ...
        # Add initial training data to the GP model
        self.model.append_train_data(self.train_x, self.train_y_init_scale)
        # self.model_exactGP.set_train_data(inputs=self.train_x, targets=self.train_y_init_scale, strict=False) # updata the Exact GP with new sampling point
    ...
    def step(self) -> None:
        ...
        if len(self.params_history_list)<1: # at the first iteration, we use EI to sample point
            self.local_stuck = True
            
        if len(self.params_history_list)>=1 and self.local_stuck == False:
            self.train_x  = torch.cat([self.train_x, self.params]) # normalized scale
            self.train_y = torch.cat([self.train_y, f_params]) # add the new sampling point obtained by gradient ascent to the training data list
            self.train_y_scale = (self.train_y-self.train_y.mean())/self.train_y.std() # standardize
            f_params_scaled = (f_params-self.train_y.mean())/self.train_y.std() 
            self.model.append_train_data(self.params, f_params_scaled)
            self.model_exactGP.set_train_data(inputs=self.train_x, targets=self.train_y_scale, strict=False)
        ...
        if self.optimize_hyperparamters:
            ...
            mll = gpytorch.mlls.ExactMarginalLogLikelihood(
                self.model_exactGP.likelihood, self.model_exactGP
            )
            botorch.fit.fit_gpytorch_model(mll)
        ...
        print(
            f"Jacobian GP lengthscale: {self.model.covar_module.base_kernel.lengthscale.detach().numpy()}, outputscale: {self.model.covar_module.outputscale.detach().numpy(): .2f},  noise {self.model.likelihood.noise.detach().numpy()}"
        )
        print(
            f"Exact GP lengthscale: {self.model_exactGP.covar_module.base_kernel.lengthscale.detach().numpy()}, outputscale: {self.model_exactGP.covar_module.outputscale.detach().numpy(): .2f},  noise {self.model_exactGP.likelihood.noise.detach().numpy()}"
        )
        ...
        if len(self.params_history_list)<=10: # use EI to sample point in the first 10 iteration
            self.local_stuck = True
            exploring = True
            UCB = False
            self.acquisition_fcn_local_stuck = botorch.acquisition.analytic.ExpectedImprovement
            self.optimize_acqf_local_stuck = optimize_acqf_vanilla_bo
            self.params = self.train_x[torch.argmax(self.train_y)].unsqueeze(0)
            inner_iteration = 1
        
        # if len(self.params_history_list)>11:
        #     #print('two norm=',torch.norm(self.params-self.params_history_list[-2], p=2))
        #     print('gradient=',torch.norm(self.params_grad, p=2))
        #     if torch.norm(self.params_grad, p=2)<=0.01: # if the sampling point for this iteraiton is too close to the previous one
                
        #         self.local_stuck = True
        #         UCB = True
        #         exploring = False
        #         # self.acquisition_fcn_local_stuck = botorch.acquisition.analytic.UpperConfidenceBound
        #         # self.optimize_acqf_local_stuck = optimize_acqf_vanilla_bo
        #         inner_iteration = 1
        #         print('stuck in local!')
        #     else:
        #         self.local_stuck = False
        #         inner_iteration = 2
        ...
        for i in range(inner_iteration):
            ...
            if self.local_stuck:
                self.bounds = torch.tensor([[0]*self.D,[1]*self.D], dtype=torch.float64)
                best_f = self.model_exactGP.train_targets.max()
                
                self.func_eval_iteration-=1
                if UCB==True:
                    new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model_exactGP,best_f=best_f), self.bounds)
                    #new_x = self.params
                    # new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model,beta=16), self.bounds) # use UCB as acquisition function
                    # self.params = new_x
                    # new_x = torch.rand((1,self.D), dtype=torch.float64)[0].unsqueeze(0)
                    print('local stuck! Start from a newly randomly sampled point:', new_x)
                else:
                    torch.manual_seed(100)
                    # new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model,beta=16), self.bounds) # use UCB as acquisition function
                    # self.params = new_x
            ...
            self.model.append_train_data(new_x, new_y_scaled) # Update the Jacobian GP
            self.model_exactGP.set_train_data(inputs=self.train_x, targets=self.train_y_scale, strict=False) # updata the Exact GP with new sampling point
            ...
            # Stop sampling if differece of values of acquired points is smaller than a threshold.
            # Equivalent to: variance of gradient did not change larger than a threshold.
            if self.epsilon_diff_acq_value is not None and self.local_stuck==False:
                if acq_value_old is not None:
                    diff = acq_value - acq_value_old
                    if diff < self.epsilon_diff_acq_value:
                        #if self.verbose:
            ...
            print('new sampling point:',self.params)
            
        if self.local_stuck==False:
            self.move(method="step") # update params by gradient ascent
        # else:
        #     self.params = new_x
...
class BayesianGradientAscent_custom1(AbstractOptimizer):
    """Optimizer for Bayesian gradient ascent.
    ...
    def __init__(
        ...
        """Inits optimizer Bayesian gradient ascent."""
        super(BayesianGradientAscent_custom1, self).__init__(params_init, objective)
        ...
        self.alpha_star = 0
        self.sampled_alpha_star = 0
        self.local_number = 0 # number of times that f_best doesnt improve
    ...
    def step(self) -> None:
        ...
        self.f_best = max(self.train_y) # current best point
        acq_value_old = None
        ...
        inner_iteration = 5
        ######### use EI to sample point in the first 10 iteration#########
        if len(self.params_history_list)<=10: 
            self.local_stuck = True
            exploring = True
            UCB = False       
            self.params = self.train_x[torch.argmax(self.train_y)].unsqueeze(0)
        ...
        #     #print('two norm=',torch.norm(self.params-self.params_history_list[-2], p=2))
        #     print('gradient=',torch.norm(self.params_grad, p=2))
        #     if self.local_number>=5: # if the sampling point for this iteraiton is too close to the previous one
        ...
        while i < inner_iteration:
            ...
            if self.local_stuck:
                ...
                if UCB==True:
                    ...
                    new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model,beta=4), self.bounds) # use UCB as acquisition function
                    print('local stuck! Start from a newly randomly sampled point:', new_x)
                else:
                    ...
                    torch.manual_seed(100)
                    # new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model,beta=16), self.bounds) # use UCB as acquisition function
                    new_x,acq_value = self.optimize_acqf_local_stuck(self.acquisition_fcn_local_stuck(self.model_exactGP,best_f=best_f), self.bounds) # use EI as acquisition function
                    print('local stuck! Start from a newly randomly sampled point:', new_x)
            ...
            if self.local_stuck and exploring == False:
                if new_y<self.f_best: # keep using EI or UCB if the sampling point doesn't improve
                    i-=1
                else:
                    self.params = new_x
                    self.local_number = 0
            ...
            if self.local_stuck:
                ...
                if self.optimize_hyperparamters:
                    ...
                    print(
                        f"lengthscale: {self.model_exactGP.covar_module.base_kernel.lengthscale.detach().numpy()}, outputscale: {self.model_exactGP.covar_module.outputscale.detach().numpy()},  noise {self.model_exactGP.likelihood.noise.detach().numpy()}"
                    )
                    mll = gpytorch.mlls.ExactMarginalLogLikelihood(
                        self.model.likelihood, self.model
                    )
        ...
        if self.local_stuck==False: # apply gibo
            ...
            if f_params<self.f_best and torch.norm(self.params_grad, p=2)<=0.01:
                self.local_number+=1
            else:
                self.local_number=0
...
class BayesianGradientAscent_custom2(AbstractOptimizer):
    """Optimizer for Bayesian gradient ascent.
    ...
    def __init__(
        ...
        """Inits optimizer Bayesian gradient ascent."""
        super(BayesianGradientAscent_custom2, self).__init__(params_init, objective)
...
class MPDOptimizer(AbstractOptimizer):
    ...
    def most_likely_uphill_direction(self, cand_params):
        ...
        pred_grad_L = psd_safe_cholesky(pred_covar, max_tries=10).unsqueeze(0)
    ...
    def step(self) -> None:
        ...
        f_params = self.objective(self.params_rescale) 
        self.func_eval_iteration +=1
        #f_params = self.objective(self.params)+ torch.randn_like(self.objective(self.params)) * np.sqrt(1e-2) 
        self.f_params = f_params
        try:
            self.f_reward = [*self.objective._func.timesteps_to_reward.values()][-1]
        except:
        ...
        if self.update_bounds:
            self.bounds = torch.tensor([[-self.delta], [self.delta]]) + self.params
        self.bounds[self.bounds<0] = 0 # the sampling point should lie within the bound
        self.bounds[self.bounds>1] = 1 # sampling point should lie within the bound
        # Only optimize model hyperparameters if N >= N_max.
        # if self.optimize_hyperparamters and (
        #     self.model.N >= self.model.N_max
        # ):  # Adjust hyperparameters
    ...
    def step_old(self) -> None:
        ...
        for i in range(self.max_samples_per_iteration):
            # Optimize acquistion function and get new observation.
            new_x, acq_value = self.optimize_acqf(self.acquisition_fcn, self.bounds)
            new_y = -self.objective(new_x) # add the minus sign if doing minimization
    ...
    def move(self, method):
        if method == "step":
            ...
            with torch.no_grad():
                ...
                num_iters = 0
                while p_star >= 0.65 and num_iters <= 500:
                    if False:
                    ...
                    for element in tmp_params.flatten(): # if the sampling point is outside the bound, break the loop
