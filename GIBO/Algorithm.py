...
class GIBO(): # This class contains all gradient-based algorithm
    def __init__(self, objective, dim, dim_true, delta, lr, epsilon_diff_acq_value, BATCH_SIZE,lb,ub, Ninit, reward_list, cost_list):
    ...
    def __call__(self, X, Y, params):
        ...
        # inner loop for GIBO
        acq_value_old = None
        max_samples_per_iteration = self.dim_true
        for i in range(max_samples_per_iteration):
            ...
            new_y = self.objective(self.lb + (self.ub - self.lb) *new_x[..., :self.dim_true])
