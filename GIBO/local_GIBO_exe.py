...
if __name__ == "__main__": 
    ...
    for seed in range(replicate):
        ...
        if RL: # Cartpole Problem
            dim = 4 # total dim including dum dimension
            dim_true = dim # true (useful) dim
            np.random.seed((replicate_list[seed])*1) 
            
            if gibo:
                N_func_evaluation = 10
                N_INIT = 5 # initial training data point                             
            ...
            lb = -1*torch.ones(dim_true) # lower bound for the variables
            ub = 1*torch.ones(dim_true) # upper bound for the variables
            objective = RL_fun(dim = dim, LVGP=False, negate=False) # define objective function
            
        elif rosenbrock:
            dim = 6 # total dim including dum dimension
            dim_true = dim # true (useful) dim
            if gibo:
                N_INIT = 4 # initial training data point
                N_func_evaluation = 10
            else:
                N_INIT = 0
                N_func_evaluation = 15
            ...
            init_x = torch.tensor(0.2+0.6*np.random.rand(1,dim)) # for Rosenbrock and Piston
            np.random.seed((seed)*1)
            X = torch.tensor(np.random.rand(N_INIT,dim))
            lb = 0*torch.ones(dim_true)
            ub = 2*torch.ones(dim_true)
            objective = Rosenbrock(dim=dim, negate=True)
            
        elif otl:
            dim = 5 # total dim including dum dimension
            dim_true = dim # true (useful) dim
            if gibo:
                N_INIT = 9 # initial training data point
                N_func_evaluation = 10
            else:
                N_INIT = 0
                N_func_evaluation = 15
        ...
        ############################################################################################################################
        if gibo:
            BO1 = GIBO(objective, dim, dim_true, delta=delta, lr = lr, epsilon_diff_acq_value=0.1, BATCH_SIZE = BATCH_SIZE, lb = lb, ub = ub,  Ninit = N_INIT, reward_list = [float(Y[0])], cost_list = [len(X)])
        else:
        ...
        while fun_evaluation<N_func_evaluation: 
            ...
            print('# function evaluation=', fun_evaluation)
