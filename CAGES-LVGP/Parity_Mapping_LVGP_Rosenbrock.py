...
N_l1 = 20 # number of data for the first level
N_l2 = 20
# N_l3 = 5
# N_l4 = 5
...
##############################################################################################################################################################
# Plotting (Parity)
plt.scatter(Y_te, y_pred, label='LVGP')
...
for i in range(len(xx)):
    plt.scatter(xx[i], yy[i], color=colors[i], s=80)
    # Adding text labels for each point
    plt.text(xx[i]-0.003, yy[i]+0.02, r'$\ell_{'+str(i)+'}$', color='black', fontsize=15)
