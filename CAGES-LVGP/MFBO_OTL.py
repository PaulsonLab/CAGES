...
class FlexibleFidelityCostModel(DeterministicModel):
    def __init__(
        ...
        fidelity_dims: list = [-1],
         values = {'4.0':1, '3.0': 10, '2.0':100, '1.0': 1000},
         fixed_cost: float = 0,
         )->None:
...
def generate_initial_data(seed = None, fun = None, dim=-1):
    ...
    X_l2_normalized = np.random.rand(N_l2,dim)
    X_l2 = lb+(ub-lb)*(X_l2_normalized) # generate initial training data (at level2) for GP
    ...
    X_l3_normalized = np.random.rand(N_l3,dim)
    X_l3 = lb+(ub-lb)*(X_l3_normalized) # generate initial training data (at level3) for GP
    ...
    X_l4_normalized = np.random.rand(N_l4,dim)
    X_l4 = lb+(ub-lb)*(X_l4_normalized) # generate initial training data (at level4) for GP
    qualatative_column = np.random.choice([4.0], size=N_l4) 
    
    if ind_qual is not None:
        X_l4 = np.column_stack((X_l4, qualatative_column))
        X_l4_normalized = np.column_stack((X_l4_normalized, qualatative_column))
      
    train_x_full= np.concatenate((X_l1, X_l2))
    train_x_full= np.concatenate((train_x_full, X_l3))
    train_x_full= np.concatenate((train_x_full, X_l4))
    ...
    train_x_full_normalized = np.concatenate((train_x_full_normalized, X_l4_normalized))
...
def optimize_mfkg_and_get_observation(mfkg_acqf, fun, dim):
    ...
    candidates, _ = optimize_acqf_mixed(
        ...
        bounds=bounds,
        fixed_features_list=[{dim: 4.0}, {dim: 3.0}, {dim: 2.0}, {dim: 1.0}],
        q=1,
        num_restarts=5,
        raw_samples=20,
...
if __name__ == '__main__':
    ...
    cost_model = FlexibleFidelityCostModel(values={'1.0':1000, '2.0':100, '3.0':10, '4.0':1}, fixed_cost=0)
    ...
    for seed in range(replicate):
        ...
        for i in range(N_ITER):
            ...
            if new_x[0][-1] == 1: # query at the highest fidelity function
                ...
                # final_rec[:,0:dim] = torch.tensor(lb)+torch.tensor(ub-lb)*final_rec[:,0:dim]
                # final_rec_obj = fun(final_rec.clone())
                # best_Y_list[seed].append(min(best_Y_list[seed][-1],float(final_rec_obj)))
                
            if cumulative_cost>15000:
                break
    ...
    padded_list = np.array([row + [0] * (max_length - len(row)) for row in cost_list])
    xx = torch.tensor(padded_list)
    
    max_length1 = max(len(row) for row in best_Y_list)
    padded_list1 = np.array([row + [0] * (max_length1 - len(row)) for row in best_Y_list])
    yy = torch.tensor(padded_list1)
    
    # Save results
    # np.save('OTL_cost_MFBO.npy',xx)
    # np.save('OTL_reward_MFBO.npy',yy)