...
class EnvironmentObjective:
    ...
    def __call__(self, params: torch.Tensor, seed=None, tau=0.02) -> torch.Tensor:
        return self.run(params, seed, tau)
    ...
    def run(
        self, params: torch.Tensor, seed, tau, render: bool = False, test: bool = False
    ) -> torch.Tensor:
        ...
        states, actions, rewards = self._unpack_episode()
        r = 0
        states[0] = self.manipulate_state(self.env.reset(seed=seed)[0]) # different from original code
        
        # if LowFidelity:
        #     self.env.unwrapped.update_tau(tau)
        # else:
        #     self.env.unwrapped.update_tau(tau)
        ...
        for t in range(self.max_steps):  # rollout
            actions[t] = self.policy(states[t].to(torch.float64), params)
            
                
            state, rewards[t], done, _ ,_= self.env.step(actions[t].numpy()) # different from original code
            # if LowFidelity:
            #     state+= np.random.normal(0,0.01,4)
                # state[1] = state[1] +  np.random.normal(0,0.1,1)
                # state[2] = state[2] +  np.random.normal(0,0.01,1)
                # state[3] = state[3] +  np.random.normal(0,0.1,1)
