...
class GradientInformation(botorch.acquisition.AnalyticAcquisitionFunction):
    ...
    def forward(self, thetas: torch.Tensor) -> torch.Tensor:
        ...
        for theta in thetas:
            ...
            # variance_d = self.model._get_Kxx_dx2()-K_xX_dx @ K_XX_inv @ K_xX_dx.transpose(1, 2) # variance at the current point if query at theta*
...
class GradientInformation_entropy(botorch.acquisition.AnalyticAcquisitionFunction):
    """Acquisition function to sample points for gradient information.
    ...
    def forward(self, thetas: torch.Tensor) -> torch.Tensor:
        ...
        for theta in thetas:
            ...
            # variance_d = self.model._get_Kxx_dx2()-K_xX_dx @ K_XX_inv @ K_xX_dx.transpose(1, 2) # variance at the current point if query at theta*
            variance_d = self.model._get_Kxx_dx2()-K_xX_dx @ K_XX_inv @ K_xX_dx.transpose(1, 2)
            log_det = 2 * torch.linalg.cholesky(variance_d).diagonal(dim1=-2, dim2=-1).log().sum(-1) # Calculate the log determinant
            variances.append(log_det)

        return -torch.cat(variances, dim=0)
    
class GradientInformation_variance(botorch.acquisition.AnalyticAcquisitionFunction):
    """Acquisition function to sample points for gradient information.
    ...
    def forward(self, thetas: torch.Tensor) -> torch.Tensor:
        ...
        for theta in thetas:
            ...
            K_XX = self.model.covar_module(X, X).evaluate() # covariance matrix K(X,X)
            K_XX = K_XX + sigma_n * torch.eye(K_XX.shape[-1]) # add jitter
            K_XX_inv = torch.inverse(K_XX)
            u = torch.linalg.cholesky(K_XX)
            K_XX_inv = torch.cholesky_inverse(u)
            
            K_xX_dx = self._get_KxX_dx(theta, X) # calculate derivative of kernel w.r.t. current theta
